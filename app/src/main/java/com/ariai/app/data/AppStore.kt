...
class AppStore(context: Context) {
    ...
    fun providers(): List<Provider> = parseArr("providers") { o ->
        Provider(o.getString("id"), o.getString("name"), o.optString("baseUrl"), o.optString("model"), o.optString("apiKey"))
    }

    fun saveProviders(list: List<Provider>) = saveArr("providers", list) { x ->
        JSONObject().put("id", x.id).put("name", x.name).put("baseUrl", x.baseUrl).put("model", x.model).put("apiKey", x.apiKey)
    }

    fun assistants(): List<Assistant> {
        val list = parseArr("assistants") { o -> Assistant(o.getString("id"), o.getString("name")) }
        return if (list.isEmpty()) {
            val d = listOf(Assistant("a1", "Default Assistant"), Assistant("a2", "Default Assistant"))
            saveAssistants(d); d
        } else list
    }

    fun saveAssistants(list: List<Assistant>) = saveArr("assistants", list) { x ->
        JSONObject().put("id", x.id).put("name", x.name)
    }
    ...
    fun conversations(): List<Conversation> {
        val list = parseArr("conversations") { o ->
            ...
            for (j in 0 until mArr.length()) {
                ...
                msgs += ChatMessage(m.getString("id"), ChatMessage.Role.valueOf(m.getString("role")), m.getString("text"), m.optString("attachment").ifBlank { null })
            }
    ...
    private fun persist(list: List<Conversation>) = saveArr("conversations", list) { c ->
        ...
        c.messages.forEach { m ->
            msgs.put(JSONObject().put("id", m.id).put("role", m.role.name).put("text", m.text).put("attachment", m.attachmentName ?: ""))
        }
        JSONObject().put("id", c.id).put("title", c.title).put("preview", c.preview).put("updatedAt", c.updatedAt).put("messages", msgs).put("providerId", c.providerId ?: "")
    }
